...
void import_data_from_csv_file_to_existing_table(ifstream& f, vector<Table>& t, int poz) {
	...
	while (getline(f, aux1))
	{
		final = "";
		int nrArgTotal = 0;
		string* command = split_string_into_words(aux1);
		string aux2;
		...
		command = split_string_into_words(final);
		identify_command_type(command, t);
		//delete[] command; cumva da eorare now sure why doesn;t make sense to me
	}
}
